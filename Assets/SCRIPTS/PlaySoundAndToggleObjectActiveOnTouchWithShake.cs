...
public class PlaySoundAndToggleObjectActiveOnTouchWithShake : MonoBehaviour
{
    ...
    void ToggleObjects()
    {
        ...
        foreach (GameObject obj in objectsToToggle)
        {
            if (obj != null) // Ensure the object is not null
            {
                ...
                Debug.Log($"{obj.name} is now {(obj.activeSelf ? "active" : "inactive")}.");
            }
            else
            {
                Debug.LogWarning("A GameObject in the toggle list is null.");
            }
        }
    }
    ...
    IEnumerator FocusOnObject()
    {
        ...
        foreach (GameObject obj in objectsToToggle)
        {
            if (obj != null) // Check if the object is not null before destroying
            {
    ...
    private IEnumerator ShakeCamera(float duration, float magnitude)
    {
        ...
        while (elapsed < duration)
        {
            ...
            playerCamera.localPosition = new Vector3(x, y, originalPosition.z);
